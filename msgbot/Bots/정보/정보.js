...
function response(room, msg, sender, isGroupChat, replier, ImageDB, packageName, threadId){
   if(msg.indexOf('/정보 ') == 0){
      ...
      var data2 = url.split('bg-navy text-white">')[1].split('더시드')[0].replace(/(<([^>]+)>)/g,"");
      ...
      if (data2.indexOf('기록이없습니다.') != -1)
         ...
         var cri_date = data2.split('기준일:')[1].split('일')[0].replace(/[가-힇]/gi,'/');
         floor = floor + ' (' + time + ') - ' + cri_date;
      }
