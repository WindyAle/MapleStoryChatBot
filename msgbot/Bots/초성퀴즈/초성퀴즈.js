...
var url = org.jsoup.Jsoup.connect("https://windyale.github.io/MapleDB").get();
url = String(url);
...
const topicRange = [0, 0, 0, 931, 698, 1578, 384];
...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName) {
   if(msg.indexOf('/퀴즈') != -1){            
      ...
      if(msg.length > 3){
        ...
        pick = Math.floor(Math.random()*topicRange[topic+2]) + topicRange[topic+1] + topicRange[topic] + topicRange[topic-1] + 1;
      }
      else      
        pick = Math.floor(Math.random()*3591) + 1;
          
      answerQuiz = url.split('이름: ')[pick].split(' 초성')[0];
      printQuiz = url.split('초성: ')[pick].split(' 범주')[0];     
      category  = url.split('범주: ')[pick].split(' 힌트')[0];
      hint = url.split('힌트: ')[pick].split('\n')[0];
